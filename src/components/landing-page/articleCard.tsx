...
const ArticleCard = ({ icon, heading, description, href = "/" }: ArticleCardProps) => {
  const icons: ReactElement[] = [
    <ChartIcon key="chart" />,
    <MagnifyIcon key="magnify" />,
    <PieChartIcon key="pie" />,
  ];

  return (
    <Link
      href={href}
      className="bg-background/80 p-6 rounded-xl shadow-sm w-full text-left hover:shadow-md transition"
    >
      <div className="flex items-center gap-3 mb-4">
        ...
        <h3 className="text-[20px] md:text-2xl font-bold text-primary">
          {heading}
        </h3>
      </div>
